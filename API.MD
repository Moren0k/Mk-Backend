...
API pública **sin autenticación** (no requiere token, cookie ni `Authorization`). Toda la información de este documento fue **verificada el 2026-08-01** contra la API real, usando la mesa Bac Bo de Evolution que ya validamos en el test en vivo (`test-evolution.cjs`).
...
- El juego Bac Bo está dentro del casino Evolution:
...
- El `data` del SSE **NO incluye** `version` ni `externalId` (solo `uuid`, `type`, `result`, `instant`).
