...
- **Persistencia (PostgreSQL/Supabase)**: `PersistenceModule`/`PrismaService` (`infrastructure/persistence/`) no participan del flujo de eventos ni del ciclo Strategy→Operation→Notification. Dos tablas reales hoy (ver `DATABASE.md` para esquema completo y decisiones de diseño):
  - `jugadas` (modelo `Jugada`): fuente de verdad histórica para el futuro motor de análisis de patrones. Todavía no hay un servicio que la pueble (`GameEventCollector` sigue escribiendo solo en el `HistoryStore` en memoria); cuando se implemente, no debe tocar `StrategyCoordinator`, `OperationCoordinator` ni `NotificationCoordinator`.
