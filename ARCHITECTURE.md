...
- **Persistencia (PostgreSQL/Supabase)**: `PersistenceModule`/`PrismaService` (`infrastructure/persistence/`) no participan del flujo de eventos ni del ciclo Strategy→Operation→Notification. **Seis tablas reales** hoy (ver `DATABASE.md` para esquema completo y decisiones de diseño):
  - `jugadas` (modelo `Jugada`): fuente de verdad histórica. **La puebla `Mk-Ingestion-Service`**, un servicio aparte con su propio repositorio, por lotes e idempotentemente (`UNIQUE(uuid)` + `skipDuplicates`). `Mk-Backend` **no escribe** en ella: su `GameEventCollector` sigue manteniendo solo la ventana en memoria de 200 jugadas (`HistoryStore`), que es lo que consume el motor de alertas.
  - **`columnas`, `racha3_operaciones`, `analytics_checkpoints`, `analytics_ejecuciones`** (+ 1 vista y 17 funciones SQL): dominio derivado de Analytics histórico "Racha 3". Es un **segundo dominio con su propio disparador** — un tick de 60 s (`Racha3IncrementalScheduler`), no el `DomainEventBus`. No conoce `StrategyCoordinator`, `OperationCoordinator` ni `NotificationCoordinator`, y ninguno de ellos lo conoce; si falla o queda sin base de datos, el motor de alertas no se ve afectado. **Analytics describe el pasado y nunca decide una alerta.** Referencia completa en [`ANALYTICS.md`](./ANALYTICS.md).
  - `report_checkpoints` (modelo `ReportCheckpoint`): sí tiene un consumidor real. `SummaryReportService` (`won`/`lost`/`alertsSent`/`uptimeMs` de `GET /api/v1/reports/summary`) vive en memoria vía `InMemoryOperationReportStore` y no sobrevive un reinicio/redeploy del proceso; `ReportCheckpointScheduler` guarda un checkpoint de esos contadores cada `REPORT_CHECKPOINT_INTERVAL_MS` (default 10 min), y `SummaryReportService.hydrateFromCheckpoint()` lo restaura al arrancar — invocado explícitamente en `main.ts`, **antes** de `GameEventCollector.start()`, mismo criterio de "explícito, no `OnModuleInit`" que el propio collector (ver más arriba en este documento): así ninguna operación real puede cerrarse y contarse antes de que el offset esté cargado.

  Si `DATABASE_URL` no está configurada o la conexión falla, `PrismaService.onModuleInit()` no lanza: la persistencia (ambas tablas) queda deshabilitada sin afectar al resto del motor — `ReportCheckpointStore.loadAll()` devuelve `[]` y `save()` solo loguea un `warn`, así que sin base de datos el comportamiento es idéntico al de antes del checkpoint (won/lost/alertsSent/uptime vuelven a cero en cada reinicio).
