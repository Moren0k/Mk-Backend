...
import { Streak4Strategy } from './streak4.strategy';
...
describe('Streak4Strategy', () => {
  let strategy: Streak4Strategy;

  beforeEach(() => {
    strategy = new Streak4Strategy();
  });
  ...
  it('recommends BANKER after four consecutive PLAYER wins', () => {
    const games = [
  ...
  it('recommends PLAYER after four consecutive BANKER wins', () => {
    const games = [
  ...
  it('does not trigger with only three consecutive equal winners', () => {
    const games = [
      buildGame('1', WinnerType.PLAYER),
      buildGame('2', WinnerType.PLAYER),
      buildGame('3', WinnerType.PLAYER),
  ...
  it('does not trigger when the last four winners are not all equal', () => {
    const games = [
      buildGame('1', WinnerType.PLAYER),
      buildGame('2', WinnerType.PLAYER),
      buildGame('3', WinnerType.PLAYER),
  ...
  it('does not trigger on four consecutive TIE results', () => {
    const games = [
  ...
  it('does not trigger with fewer than four games in history', () => {
    const games = [
      buildGame('1', WinnerType.PLAYER),
      buildGame('2', WinnerType.PLAYER),
      buildGame('3', WinnerType.PLAYER),
  ...
  it('only evaluates the current streak, ignoring older unrelated history', () => {
    ...
    if (result.triggered) {
      expect(result.recommendedWinner).toBe(WinnerType.BANKER);
      expect(result.streakWinner).toBe(WinnerType.PLAYER);
      expect(result.metadata.streakGameUuids).toEqual(['3', '4', '5', '6']);
    }
  });
  ...
  describe('one signal per streak (not a sliding window)', () => {
    ...
    it('triggers again once a TIE ends the streak and a new one of length 4 forms', () => {
      const runtimeState = buildRuntimeState();
      const games = [
        buildGame('1', WinnerType.PLAYER),
        buildGame('2', WinnerType.PLAYER),
      ...
      games.push(buildGame('5', WinnerType.PLAYER));
      expect(
        strategy.evaluate(buildContext(games, { runtimeState })).triggered,
      ).toBe(false);

      games.push(buildGame('6', WinnerType.TIE));
      expect(
        strategy.evaluate(buildContext(games, { runtimeState })).triggered,
      ).toBe(false);
      ...
      games.push(buildGame('9', WinnerType.BANKER));
      expect(
        strategy.evaluate(buildContext(games, { runtimeState })).triggered,
      ).toBe(false);

      games.push(buildGame('10', WinnerType.BANKER));
      const result = strategy.evaluate(buildContext(games, { runtimeState }));
      expect(result.triggered).toBe(true);
      if (result.triggered) {
        expect(result.recommendedWinner).toBe(WinnerType.PLAYER);
        expect(result.streakWinner).toBe(WinnerType.BANKER);
        expect(result.metadata.streakGameUuids).toEqual(['7', '8', '9', '10']);
      }
    });
